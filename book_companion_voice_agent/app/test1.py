...
def load_css():
    ...
    <style>
        ...
        /* New styles for voice recording */
        .recording-container {
...
########
def record_audio_with_vad(silence_threshold=500, silence_duration=1.0, max_duration=30, sample_rate=16000):
    """
    Record audio based on voice activity detection (VAD)
    ...
    frames = []
    silent_frames = 0
    max_silent_frames = int(silence_duration * sample_rate / 1024)
    start_time = time.time()
    has_speech = False
    
    # Wait for initial speech before starting recording
    print("Waiting for speech...")
    warmup_frames = 0
    while warmup_frames < 10:  # Check a few frames to avoid false triggers
        data = stream.read(1024, exception_on_overflow=False)
        # Convert to int16 array
        int_data = array.array('h', data)
        # Calculate RMS of frame
        rms = np.sqrt(np.mean(np.array(int_data)**2))
    ...
    while True:
        ...
        if time.time() - start_time > max_duration:
            print(f"Maximum recording duration of {max_duration}s reached")
            break
        ...
        if rms < silence_threshold:
            ...
            # Reset silent frames counter when speech detected
            silent_frames = 0
...
def calibrate_microphone(duration=3, sample_rate=16000):
    """
    ...
    Returns:
        Recommended silence threshold value
    """
    try:
        st.info("Please remain silent during microphone calibration...")
        progress_bar = st.progress(0)
        ...
        rms_values = []
        frames_to_record = int(duration * sample_rate / 1024)
        ...
        ambient_noise = np.mean(rms_values)
        # Set threshold to be 2.5x the ambient noise level
        threshold = int(ambient_noise * 2.5)
        
        st.success(f"Calibration complete! Ambient noise level: {ambient_noise:.2f}, Recommended threshold: {threshold}")
        return threshold
...
# Add this to the sidebar in your main_app function
with st.sidebar:
    ...
    if st.session_state.voice_mode:
        ...
        if "silence_threshold" not in st.session_state:
            st.session_state.silence_threshold = 500
        if "silence_duration" not in st.session_state:
            st.session_state.silence_duration = 1.5
        if "max_duration" not in st.session_state:
            st.session_state.max_duration = 30
        
        # Voice settings controls
        col1, col2 = st.columns(2)
        with col1:
            if st.button("Calibrate Microphone"):
                threshold = calibrate_microphone()
                st.session_state.silence_threshold = threshold
        
        with col2:
            st.session_state.silence_threshold = st.number_input(
                "Silence Threshold", 
                min_value=100, 
                max_value=2000, 
        ...
        st.session_state.silence_duration = st.slider(
            "Silence Duration (s)", 
            min_value=0.5, 
            max_value=3.0, 
...
# Then update your start_recording function to use these parameters
def start_recording():
    ...
    with st.spinner("Recording... (speak now, will stop after silence)"):
        # Use the parameters from session state
        audio_data, sample_rate = record_audio_with_vad(
            silence_threshold=st.session_state.silence_threshold,
            silence_duration=st.session_state.silence_duration,
...
def start_recording():
    ...
    with st.spinner("Recording... (speak now, will stop after silence)"):
        # Adjust these parameters as needed for your microphone and environment
        audio_data, sample_rate = record_audio_with_vad(
            silence_threshold=500,  # Adjust based on microphone sensitivity
            silence_duration=1.5,   # Stop after 1.5 seconds of silence
            max_duration=30,        # Maximum recording of 30 seconds
            sample_rate=16000
        )
        ...
        if st.session_state.voice_mode:
            if st.session_state.recording:
                ...
                <div class="recording-container">
                    <h3><span class="recording-indicator"></span>Recording in progress</h3>
                    <p>Speak your question about the book. Recording will automatically stop after detecting silence.</p>
                    <div class="recording-timer" id="recording-timer">00:00</div>
                    <div class="voice-activity-meter">
                        <div class="voice-activity-level" id="voice-level" style="width:0%"></div>
                    </div>
                </div>
                ...
                if st.button("⏹️ Stop Recording", key="stop_recording", on_click=start_recording):
                    pass
            else:
                voice_col1, voice_col2 = st.columns([1, 4])
                with voice_col1:
                    if st.button("🎤 Record", key="record_button", on_click=start_recording):
                        pass
