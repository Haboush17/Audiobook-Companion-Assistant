...
def initialize_clients():
    ...
    if st.session_state.index_name not in st.session_state.pc.list_indexes().names():
...
def process_pdf_to_pinecone(pdf_file):
    try:
        with st.spinner("Processing book and uploading to Pinecone..."):
            ...
            total_batches = (len(documents) + 19) // 20  # Ceiling division
...
def text_to_speech(text):
    try:
        ...
        res = requests.post(DEEPGRAM_TTS_URL, headers=headers, json={'text': formatted_text})
...
def get_book_namespaces():
    ...
    try:
        ...
        # Create a simple query to check for namespace existence
        # This is a workaround since describe_stats might not be available
        namespaces = []
        
        # You could store the namespaces in a separate file or database
        # For simplicity, we'll just return any books that have been initialized in the current session
...
async def main_app():
    ...
    with st.sidebar:
        ...
        # Book list - modified to avoid describe_stats
